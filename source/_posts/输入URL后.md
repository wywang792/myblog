...
![移动终端的请求先发送到基站，接着发送到运营商的中心计算机，然后再发送到互联网](./assets/输入URL后/latency.jpg)
...
![DNS 查询、TCP 握手和 TLS 5 步握手（包括服务器和客户端之间的 clienthello、serverhello 以及证书、clientkey 和完成消息）。](./assets/输入URL后/ssl.jpg)
...
![我们示例代码的 DOM 树，显示了所有节点（包括文本节点）。](./assets/输入URL后/dom.gif)
...
![通过快速连接，主线程被 JavaScript 文件下载、解析和执行占用](./assets/输入URL后/visa_network.png)
